...
class TrainingTab(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield ScrollableContainer(
            Input(id="t_task_name", placeholder="ML Workflow name", type="text", value=self.ml_config['name']),
            Select(id="t_sel_algo", options=[]))
    ...
    def update_selected_view(self) -> None:
        ...
        self.ml_config['target'] = [str(select_list.get_option_at_index(selected_item).prompt) for selected_item in
...
class PredictionTab(Static):
        ...
        if len(self.predict_config['features']) > 1:
            sub_smt = "json_object(" + ", ".join(
                ...
                self.predict_config['features'][:-1]) + ',' + re.escape("'") + \
                      self.predict_config['features'][-1][0] + re.escape("'") + ', [' + \
                      self.predict_config['features'][-1][
                          0] + '])'
        else:
            sub_smt = "json_object(" + ", ".join(
                re.escape("'") + f[0] + re.escape("'") + ', [' + f[0] + '])' for f in self.predict_config['features'])
        smt_batched = """
        SELECT
          {table}.*,
    ...
    def select_changed(self, event: Select.Changed) -> None:
        ...
        f_list = [(name[0], idx, True) for idx, name in
                  enumerate(get_table_columns(self.conn, self.predict_config['dataset']))]
...
class PrePrecessingTab(Static):
    ...
    def create_new_table(self, clean=True):
        ...
        result = self.conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?",
                                   (self.table['name'],)).fetchone()
        if result:
